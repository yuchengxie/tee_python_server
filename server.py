...
class WalletApp(object):
        ...
        if r.status_code == 200:
            ...
            if msg.command == protocol.UdpReject.command:
                ...
                if msg.account == account:  # msg.link_no is peer's node._link_no
                    ...
                    json_entity = entity_2_json(info, SUCCESS)
        else:
            json_entity = entity_2_json(self.failed_desc(r), FAILED)
        return json_entity
    ...
    def block_state(self, block_hash, heights=None):  # block_hash should be str or None
        ...
        if not block_hash and not heights:
            ...
            json_entity = entity_2_json('warning: nothing to query', FAILED)
        ...
        if r.status_code == 200:
            ...
            if msg.command == protocol.UdpReject.command:
                # print('Error: ' + self.get_reject_msg_(msg))
                json_entity = entity_2_json(self.get_reject_msg_(msg), FAILED)
            elif msg.command == protocol.ReplyHeaders.command:
                if not msg.headers:
                    # print('no block is found!')
                    json_entity = entity_2_json('no block is found', FAILED)
                else:
                    blocks = []
                    for (idx, block) in enumerate(msg.headers):
                        ...
                        _block['height'] = hi
                        _block['txck'] = txck
                        _block['hash'] = hash_str(block.hash)
                        _block['version'] = block.version
                        _block['link_no'] = block.link_no
                        _block['prev_block'] = hash_str(block.prev_block)
                        _block['merkle_root'] = hash_str(block.merkle_root)
                        _block['timestamp'] = block.timestamp
                        _block['bits'] = block.bits
                        _block['nonce'] = block.nonce
                        _block['miner'] = hash_str(block.miner)
                        _block['txn_count'] = block.txn_count
...
def verify_pin(psw):
    result = {}
    if psw == None:
        return entity_2_json(MSG_NOT_EXIST_ARGS, FAILED)
    if psw == '':
        return entity_2_json(MSG_NOT_BE_EMPTY, FAILED)
    try:
        res, status = transmit(('00200000%02x' % (len(psw)//2,)) + psw)
        return entity_2_json(MSG_VERIFY_PIN_SUCCESS, SUCCESS)
    except:
        if gLastErrCode[:3] == '63c':
            return entity_2_json('incorrect PIN code, left try count: ' +
                                 gLastErrCode[-1:], FAILED)
        else:
            return entity_2_json(MSG_VERIFY_PIN_FAILED, FAILED)
...
# teeNotExistMsg = {'msg': 'tee not ready', 'status': TEENOTREADY}

MSG_TEENotExist = 'tee not ready'
MSG_NOT_EXIST_ARGS = 'not exist args'
MSG_NOT_BE_EMPTY = 'can not be empty'
MSG_VERIFY_PIN_SUCCESS = 'verify PIN code successful'
MSG_VERIFY_PIN_FAILED = 'verify PIN code failed'


def entity_2_json(msg='', status=FAILED):
    entity = {}
    entity['msg'] = msg
...
def teeNotExist():
    return entity_2_json(MSG_TEENotExist, TEENOTREADY)
...
def verifypin():
    ...
    json_entity = verify_pin(pin_code)
    # return json_entity
    return json_entity
...
def get_info():
    ...
    wallet = WalletApp(gPseudoWallet, gPseudoWallet._vcn)
    wallet.WEB_SERVER_ADDR = curr_coin.WEB_SERVER_ADDR
    json_entity = wallet.account_state(uockAfter, uockBefore)
    return json_entity
...
def get_block():
    ...
    json_entity = app.block_state(block_hash, heights)
    return json_entity
...
def get_sign():
    ...
    # 1.verify pin_codes
    wallet = WalletApp(gPseudoWallet, gPseudoWallet._vcn)
    result = json.loads(verify_pin(pin_code))
    print('result:', result)
    if(result['status'] == FAILED):
        ...
        if(status == '9000'):
            ...
            t = hexlify(t).decode()
            json_entity = entity_2_json(t, SUCCESS)
        else:
            json_entity = entity_2_json('TEE sign transaction failed', FAILED)
        return json_entity
