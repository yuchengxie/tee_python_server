...
class Message(format.CompoundType):
  ...
  def binary(self, magic):
    ...
    command = self.command.encode('utf-8')  # command must be '_a-zA-Z0-9'
...
class Transaction(Message):
  ...
  def _debug(self):
    return _debug(self, [('n_in', len(self.tx_in)), ('n_out', len(self.tx_out))])
...
class Block(Message):
  ...
  def _debug(self):
    return _debug(self, [('prev',hexlify(self.prev_block)),('tm',self.timestamp),('n',len(self.txns))])
